...
#include "fingerprint_bondweightscreened.h"
...
Fingerprint_bondweightscreened::Fingerprint_bondweightscreened(PairRANN *pair) : Fingerprint(pair)
{
	n_body_type = 3;
	dr = 0;
	re = 0;
...
Fingerprint_bondweightscreened::~Fingerprint_bondweightscreened(){
	delete [] alpha_k;
	delete [] weights;
	delete [] atomtypes;
	delete [] expcuttable;
...
bool Fingerprint_bondweightscreened::parse_values(char * constant, char * line1){
	...
	else if (strcmp(constant,"weights")==0){
		if (nwords!=pair->nelements){pair->errorf("Invalid number of weights in bondweightscreened fingerprint\n");}
		for (l=0;l<nwords;l++){
			weights[l]=strtod(words[l],NULL);
		}
	}
	...
	delete [] words;
	if (re!=0.0 && rc!=0.0 && alpha_k[0]!=-1 && dr!=0.0 && m!=0 && k!=0 && weights[0]!=-1)return true;
	return false;
}

void Fingerprint_bondweightscreened::write_values(FILE *fid){
	int i;
...
void Fingerprint_bondweightscreened::init(int *i,int id){
	...
	this->k = 0;
	delete [] weights;
	weights = new double [pair->nelements];
	for (int j=0;j<pair->nelements;j++){weights[j]=-1;}
	empty = false;
	this->id = id;
}

//number of neurons defined by this fingerprint
int Fingerprint_bondweightscreened::get_length(){
	return m*k;
}

void Fingerprint_bondweightscreened::allocate(){
...
void Fingerprint_bondweightscreened::generate_exp_cut_table(){
    int m,n;
	double r1;
	int buf = 5;
	int res = pair->res;
...
void Fingerprint_bondweightscreened::generate_coefficients(){      //calculates multinomial coefficient for each term
    int p,mb,mc;
	int n,p1,i1;
	mb = this->m;
	mc=(mb*(mb+1))>>1;
...
void Fingerprint_bondweightscreened::compute_fingerprint(double * features,double * dfeaturesx,double *dfeaturesy,double *dfeaturesz,double *Sik, double *dSikx, double*dSiky, double *dSikz, double *dSijkx, double *dSijky, double *dSijkz, bool *Bij, int ii,int sid,double *xn,double *yn,double*zn,int *tn,int jnum,int *jl){
	int i;
//	int *ilist,*numneigh;
...
int Fingerprint_bondweightscreened::factorial(int n) {
   if ((n==0)||(n==1))
      return 1;
   else
